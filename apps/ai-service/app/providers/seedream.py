...
POSTER_ORIENTATIONS = {
    "portrait": {
        "label": "竖版 3:4",
        # Seedream 要求总像素数 >= 3686400（实测报错阈值），原先的 1536x2048 只有
        # 3145728，稳定 400。等比放大到 1728x2048->1728x2304 保持 3:4 比例的同时
        # 把总像素提到 3981312，留出安全余量。
        "size": "1728x2304",
        "ratio": "3:4",
        "composition": (
    ...
    "landscape": {
        "label": "横版 16:9",
        # 原先 2048x1152 只有 2359296 像素，同样低于 Seedream 的 3686400 下限。
        # 换成 2560x1440，保持 16:9 比例且总像素刚好等于下限（跟 9:16 默认尺寸
        # 是同一组数字对调，已知能过）。
        "size": "2560x1440",
        "ratio": "16:9",
        "composition": (
    ...
    "4:3": {
        "label": "横版 4:3",
        # 原先 2048x1536 只有 3145728 像素，同样不够。换成 2304x1728（跟 portrait
        # 的 1728x2304 对调宽高，同样是 3981312 像素），保持 4:3 比例。
        "size": "2304x1728",
        "ratio": "4:3",
